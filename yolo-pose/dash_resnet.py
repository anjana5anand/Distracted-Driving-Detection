...
epochs = 10
learning_rate = 2e-6
weights_save_dir = "/media/viplab/DATADRIVE1/driver_action_recognition/resnext_pose_weights/dash"
best_val_accuracy = 0
view = "dash"
all_images_paths = "/media/viplab/Storage1/driver_action_recognition/crop_new/cut_frames_dash"
log_file_path = "/media/viplab/DATADRIVE1/driver_action_recognition/resnext_pose_weights/dash/dash_logs_resnet.txt"
...
class ResNetClassifier(nn.Module):
    def __init__(self, num_classes):
        ...
        self.num_features = self.model.fc.in_features + 34
        # print(self.num_features)
        self.classifier = nn.Linear(self.num_features, num_classes)
        self.model.fc = nn.Identity()
        
        
    def yolo_pose(self, x):
        ...
        def yolo_inference(x, yolo_model):
            x_cpu = denormalize(x.detach().cpu(), mean=0.5, std=0.5)
            batch_size = x_cpu.shape[0]
            image_stack = []

            for i in range(batch_size):
                img_np = x_cpu[i].permute(1, 2, 0).numpy()
                if img_np.max() <= 1.0:
                ...
                image_stack.append(img_pil)

            results = yolo_model(image_stack, device="cuda:0", verbose=False)
            keypoints_stack = []
            ...
            for i in range(batch_size):
                ...
                person_indices = [j for j, box in enumerate(boxes.cls) if int(box) == 0]

                if not person_indices:
                    print(f"No person detected in image {i}")
                    keypoints_stack.append(torch.zeros(34, device="cuda"))  # assuming 17 keypoints
                    continue

                # Pick most confident person detection
                best_idx = max(person_indices, key=lambda j: boxes.conf[j])

                # Get corresponding keypoints
                keypoints = keypoints_list.xy[best_idx].flatten()
                keypoints_stack.append(keypoints)

                # result.save(filename=f"result_{i}.jpg")

            try:
                keypoints_tensor = torch.stack(keypoints_stack)
            except RuntimeError:
                keypoints_tensor = pad_sequence(keypoints_stack, batch_first=True)

            return keypoints_tensor

        keypoints_stack = yolo_inference(x, yolo_model)
        return keypoints_stack


    
    def forward(self, x, yolo_model):

        feat = self.model(x)

        feat = self.model(x)
        yolo_feat = self.yolo_pose(x)
        x2 = torch.cat((feat, yolo_feat), axis = 1)
        #print(x2.shape)
...
for epoch in range(epochs):
    ...
    for frames, labels in tqdm(train_loader, desc=f"Epoch {epoch + 1}/{epochs} - Training", leave=False):
        ...
        optimizer.zero_grad()
        outputs = model(frames, yolo_model)

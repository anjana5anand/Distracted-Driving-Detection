...
train_labels_root = '/media/viplab/DATADRIVE1/driver_action_recognition/arrays'

validation_features_root = '/media/viplab/Storage1/driver_action_recognition/raw_features/A1/validation'
validation_labels_root = '/media/viplab/DATADRIVE1/driver_action_recognition/arrays'

num_epochs = 100
weights_save_path = "lstm_weights"
...
train_dataset = ChunkedVideoDataset(t_dataset, chunk_size=100)
valid_dataset = ChunkedVideoDataset(v_dataset, chunk_size=100)
...
#     def __init__(self, input_dim=6144, hidden_dim=1024, num_classes=16, seq_len=50):
#         super().__init__()
...
class LSTMSequenceModel(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers=1, bidirectional=False):
        ...
        self.output_proj = nn.Linear(out_dim, output_dim)

    def forward(self, x):
        """
        Args:
            x: Tensor of shape (batch_size, 100, input_dim)
        Returns:
            Tensor of shape (batch_size, 100, output_dim)
        """
        x = self.input_proj(x)          # (B, 100, hidden_dim)
        x, _ = self.lstm(x)             # (B, 100, hidden_dim or 2*hidden_dim)
        x = self.output_proj(x)         # (B, 100, output_dim)
        return x

def train_lstm(
    model, train_loader, val_loader, optimizer, criterion, num_epochs, weights_save_path, device,
    ...
    for epoch in range(num_epochs):
        ...
        for features, labels in tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs}"):
            ...
            optimizer.zero_grad()
            outputs = model(features)        # [B, 50, num_classes]


            outputs = outputs.view(-1, outputs.size(-1))  # (B*100, num_classes)
            ...
            # --- Accuracy ---
            preds = outputs.argmax(dim=-1)  # [B, 50]
        ...
        if val_accuracy > best_val_accuracy:
            ...
            torch.save(model.state_dict(), f"{weights_save_path}/lstm_best.pth")
        ...
        if (epoch + 1) % 5 == 0:
            # Save the model checkpoint
            torch.save(model.state_dict(), f"{weights_save_path}/lstm_epoch_{epoch+1}.pth")
            print(f"Model saved at epoch {epoch+1}")

# model = MambaSequenceClassifier(input_dim=6144, hidden_dim=256, num_classes=16)
model = LSTMSequenceModel(input_dim=6144, hidden_dim=256, output_dim=16, bidirectional=False)
optimizer = torch.optim.AdamW(model.parameters(), lr=1e-5)
criterion = nn.CrossEntropyLoss()

train_lstm(model, train_loader, valid_loader, optimizer, criterion, num_epochs, weights_save_path, device="cuda", num_classes=16)

...
train_features_root = '/media/viplab/DATADRIVE1/driver_action_recognition/pose_resnet_features_norm/A1/train'
train_labels_root = '/home/viplab/Documents/driver_action_recognition/data_processing/array_generation/arrays'

validation_features_root = '/media/viplab/DATADRIVE1/driver_action_recognition/pose_resnet_features_norm/A1/valid'
validation_labels_root = '/home/viplab/Documents/driver_action_recognition/data_processing/array_generation/arrays'

num_epochs = 100
weights_save_path = "mamba_weights_6246_iou"
...
class BoundaryAwareLoss(nn.Module):
    ...
    def forward(self, frame_logits, labels, pred_boundaries, true_boundaries):
        ...
        # ---- IoU loss for boundaries ----
        pred_start, pred_end = pred_boundaries[:, 0], pred_boundaries[:, 1]
        true_start, true_end = true_boundaries[:, 0], true_boundaries[:, 1]

        inter_start = torch.max(pred_start, true_start)
        inter_end = torch.min(pred_end, true_end)
        inter = (inter_end - inter_start).clamp(min=0)

        union_start = torch.min(pred_start, true_start)
        union_end = torch.max(pred_end, true_end)
        union = (union_end - union_start).clamp(min=1e-6)  # avoid division by zero

        iou = inter / union
        loss_loc = 1 - iou.mean()

        # ---- Final loss ----
        total_loss = self.classification_weight * loss_cls + self.localization_weight * loss_loc
        return total_loss
...
model = MambaSequenceClassifier(input_dim=6246, hidden_dim=4164, num_classes=16)

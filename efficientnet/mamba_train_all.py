...
train_features_root = '/media/viplab/DATADRIVE1/driver_action_recognition/efficientnet_features/A1/train'
train_labels_root = '/home/viplab/Documents/driver_action_recognition/data_processing/array_generation/arrays'

validation_features_root = '/media/viplab/DATADRIVE1/driver_action_recognition/efficientnet_features/A1/valid'
validation_labels_root = '/home/viplab/Documents/driver_action_recognition/data_processing/array_generation/arrays'

num_epochs = 100
weights_save_path = "mamba_effnet_weights"
...
class MultiViewFeatureDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        dash_feat, rear_feat, side_feat = view_features[0][:min_rows, :], view_features[1][:min_rows, :], view_features[2][:min_rows, :]  # [seq_len, total_feat_dim]
        label_path = os.path.join(self.labels_root, os.path.dirname(key)+ ".npy")
        labels = np.load(label_path).astype(np.int64)  # [seq_len]

        return dash_feat, rear_feat, side_feat, labels
...
class ChunkedVideoDataset(Dataset):
    def __init__(self, base_dataset, chunk_size=100, stride=50):
        ...
        for video_idx in range(len(base_dataset)):
            featured, featurer, features, labels = base_dataset[video_idx]
    ...
    def __getitem__(self, idx):
        ...
        dash_feat, rear_feat, side_feat, labels = self.base_dataset[video_idx]
        dash_chunk, rear_chunk, side_chunk = dash_feat[start:start+self.chunk_size], rear_feat[start:start+self.chunk_size], side_feat[start:start+self.chunk_size]
        labels_chunk = labels[start:start+self.chunk_size]
        return dash_chunk, rear_chunk, side_chunk, labels_chunk

train_dataset = ChunkedVideoDataset(t_dataset, chunk_size=300, stride=150)
...
for featured, featurer, features, labels in train_loader:
    print('train')
    print("Concatenated features:", featured.shape, featurer.shape, features.shape)  # [1, seq_len, total_feat_dim]
...
class MambaSequenceClassifier(nn.Module):
    def __init__(self, input_dim=1280, hidden_dim=1024, num_classes=16):
        super().__init__()
        # self.input_proj = nn.Linear(input_dim, hidden_dim)

        self.mamba_dash = Mamba(
            d_model=input_dim,
            d_state=16,
            d_conv=4,
            expand=2
        )
        self.mamba_rear = Mamba(
            d_model=input_dim,
            d_state=16,
            d_conv=4,
            expand=2
        )
        self.mamba_side = Mamba(
            d_model=input_dim,
            d_state=16,
            d_conv=4,
            expand=2
    ...
    def forward(self, x1, x2, x3):
        ...
        """
        # x1 = self.input_proj(x1)  # -> [B, L, H]
        # x2 = self.input_proj(x2)
        # x3 = self.input_proj(x3)

        x1 = self.mamba_dash(x1)  # [B, L, H]
        x2 = self.mamba_rear(x2)
        x3 = self.mamba_side(x3)
...
def train_mamba(
    ...
    for epoch in range(num_epochs):
        ...
        for featured, featurer, features, labels in tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs}"):
            ...
            featurer = featurer.to(device)
            featured = featured.to(device)
...
model = MambaSequenceClassifier(input_dim=1280, hidden_dim=1024, num_classes=16)

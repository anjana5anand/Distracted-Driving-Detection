...
epochs = 10
learning_rate = 2e-6
weights_save_dir = "/media/viplab/DATADRIVE1/driver_action_recognition/resnet_weights/rear"
best_val_accuracy = 0
view = "rear"
all_images_paths = "/media/viplab/Storage1/driver_action_recognition/crop_new/cut_frames_rear"
log_file_path = "/media/viplab/DATADRIVE1/driver_action_recognition/resnet_weights/rear/rear_logs_resnet.txt"
...
for epoch in range(epochs):
    ...
    if val_accuracy > best_val_accuracy:
        ...
        best_epoch = epoch + 1
        torch.save(model.state_dict(), weights_save_dir + '/' + f"Resnet_{view}_best.pth")
        print(f"New best model saved at epoch {best_epoch} with validation accuracy: {best_val_accuracy:.2f}%")

    if (epoch + 1) % 5 == 0:
        torch.save(model.state_dict(), weights_save_dir + '/' + f"Resnet_{view}_{epoch + 1}_{learning_rate}.pth")

    # Final Save
    torch.save(model.state_dict(), weights_save_dir + '/' + f"Resnet_final.pth")
    print(f"Training complete. Best validation accuracy: {best_val_accuracy:.2f}% at epoch {best_epoch}")
    os.system(f'echo "Best rear Weight {best_epoch}" > best_rear_epoch.txt')
